...
y0 = measure(x(:,1),Pd);
b = (rand()-0.5)*360;
X0 = [y0(1),y0(2),0,0,y0(3), y0(4)*cosd(b), y0(4)*sind(b), b,50*cosd(b),50*sind(b),b]';
ekf = EKF(X0,Dt);
...
for k=2:length(t)
    ...
    % =============== Filter ==============================================
    ekf = ekf.compute(y(8:9,k), y(1:7,k));
    x_hat(:,k) = ekf.X;
    debug(:,k) = ekf.debug;
    ...
    % ============ Compute control ========================================
    controller.compute(x_hat(:,k), y(:,k), dock_pos_cov(:,k));
...
plot([P0(2),P0(2)+30*sind(b)], [P0(1),P0(1)+30*cosd(b)], 'r--', 'LineWidth', 2);
